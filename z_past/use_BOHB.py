...
## training part
def start_training(masklist, n_iterations):
...
hyperparameter_list = []
pixel_list = [i for i in range(28*28)]
for i in range(40):
    hyperparameter_list.append(cs.CategoricalHyperparameter(str(i), pixel_list))

configspace = cs.ConfigurationSpace(hyperparameter_list)
opt = BOHB(configspace, start_training, max_budget=100, min_budget=10)
logs = opt.optimize()
print(logs)


for i in range(10):
    plt.plot(spectral_data_list[i],".")
plt.show()
