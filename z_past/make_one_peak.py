...
def start_training(masklist):
    ...
    predictions = []
    for selected_feature in selected_features[:10]:
    ...
    for prediction in predictions:
        ...
        #result_peak = sp.signal.find_peaks(prediction_result)[0]
        # result += 1-max(prediction_result[result_peak])
        result += len([x for x in prediction_result if x >= 0.01])

    spectral_data_list = np.array(spectral_data_list)

    return result

guess = [random.randint(0,28*28-1) for i in range(20)]
print(f"This is the guess for the mask: {guess}")


res = sp.optimize.basinhopping(start_training, guess,stepsize=50)
print(res)
print(res.x)
...
print(sp.signal.find_peaks(spectral_data_list[0])[0])

for i in range(5):
    print(len([x for x in spectral_data_list[i] if x>=0.01]))
